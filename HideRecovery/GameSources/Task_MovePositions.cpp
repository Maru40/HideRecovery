...
namespace basecross {
	...
	Task_MovePositions_Parametor::Task_MovePositions_Parametor() :
		Task_MovePositions_Parametor(std::make_shared<Task_ToTargetMove::Parametor>(
			Vec3(0.0f), Vec3(0.0f), 3.0f, 2.0f, MoveType::SeekVelocity, DeltaType::Normal))
	{}

	Task_MovePositions_Parametor::Task_MovePositions_Parametor(const Task_ToTargetMove::Parametor& moveParametor)
		: Task_MovePositions_Parametor(std::make_shared<Task_ToTargetMove::Parametor>(moveParametor))
	{}

	Task_MovePositions_Parametor::Task_MovePositions_Parametor(
		const std::shared_ptr<Task_ToTargetMove::Parametor>& moveParamPtr
	):
	...
	void Task_MovePositions::DefineTask() {
		//ターゲットに向かう処理
		m_taskList->DefineTask(TaskEnum::Move, std::make_shared<Task_ToTargetMove>(GetOwner(), m_paramPtr->moveParamPtr));
	}
