...
namespace basecross {
	namespace Enemy {
		...
		Task_PlowlingMove::Parametor::Parametor()
			:Parametor(std::make_shared<Task::Task_ToTargetMove::Parametor>())
		{}

		Task_PlowlingMove::Parametor::Parametor(const std::shared_ptr<Task::Task_ToTargetMove::Parametor>& moveParamPtr)
		...
		void Task_PlowlingMove::DefineTask() {
			//目的地に向かって移動する処理
			m_taskList->DefineTask(TaskEnum::ToTargetMove, std::make_shared<Task::Task_ToTargetMove>(GetOwner()->GetGameObject(), m_paramPtr->moveParamPtr));
		}
