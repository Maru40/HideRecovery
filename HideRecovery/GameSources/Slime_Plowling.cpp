...
/*!
@file Slime_Plowling.cpp
@brief Slime_Plowlingのクラス実体
担当：丸山裕喜
*/
...
namespace basecross {
	namespace Enemy {
		...
		namespace StateNode {
			...
			Slime_Plowling::Parametor::Parametor() :
			...
			enum class Slime_Plowling::TaskEnum
			{
				Move,  //移動
			...
			Slime_Plowling::Slime_Plowling(const std::shared_ptr<EnemyBase>& owner, const std::shared_ptr<Parametor>& paramPtr)
				:EnemyStateNodeBase<EnemyBase>(owner), m_paramPtr(paramPtr), m_taskList(new TaskList<TaskEnum>()), m_arriveTimer(new GameTimer(0.0f))
			{
				DefineTask();
			}

			void Slime_Plowling::OnStart() {
			...
			void Slime_Plowling::DefineTask() {
				auto enemy = GetOwner();
			...
			void Slime_Plowling::SelectTask() {
				TaskEnum tasks[] = {
					TaskEnum::Move,
					TaskEnum::Wait
				};
			...
			void Slime_Plowling::NextPosition() {
				//DebugDrawで元の色に戻す。
				ChangeDebugNodeColor(Col4(1.0f));
			...
			void Slime_Plowling::ChangeMoveTargetPosition(const Vec3& position) {
			...
			void Slime_Plowling::CalculateAstarRamdomPosition() {
				//Astarからポジション群を取得
			...
			float Slime_Plowling::CalculateArriveTime(const Vec3& targetPosition, const float& speed) const {
			...
			void Slime_Plowling::ChangeDebugNodeColor(const Col4& color) {
